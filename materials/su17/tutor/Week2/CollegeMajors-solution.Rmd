...
bigmajors = collegedata.where("Total", are.above(50000))
...
```{python}
dropped = bigmajors.drop("Major_code", "Employed_full_time_year_round")
dropped
```
...
employsorted = ...
categorysorted = ...
bothsorted = dropped.sort("Unemployment_rate").sort("Major_category")
bothsorted.show()
```
...
```{python}
business = bothsorted.take(np.arange(18,30))
business.select(0, 5).barh("Major")
```
...
Create and add a new column called employment rate to bothsorted. (This is employed/(unemployed + employed)). Assign this new table to the same name as before, bothsorted.

```{python}
bothsorted = bothsorted.with_column("Employment_rate", bothsorted.column("Employed")/(bothsorted.column("Unemployed") + bothsorted.column("Employed")))
```
...
Here's something a little more useful; is there a relationship between unemployment rate and median income? Plot the two and find out.
